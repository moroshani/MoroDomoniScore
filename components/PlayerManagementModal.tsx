...
export const PlayerManagementModal: React.FC<PlayerManagementModalProps> = ({ isOpen, onClose, onPlayersUpdate }) => {
  ...
  const handleAddNewPlayer = () => {
    if (newPlayerName.trim() && !players.some(p => p.name === newPlayerName.trim())) {
      const newPlayer: Player = { id: newPlayerName.trim(), name: newPlayerName.trim(), avatar: newPlayerAvatar || '👤' };
    ...
    <>
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4" role="dialog" aria-modal="true">
        <div ref={focusTrapRef} className="glass-card rounded-3xl p-8 max-w-md w-full">
          <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-6">مدیریت بازیکنان</h2>
          
          <div className="space-y-4 max-h-60 overflow-y-auto pr-2 mb-6">
            {players.map(player => (
              <div key={player.id} className="flex items-center justify-between bg-gray-500/10 dark:bg-gray-900/20 p-3 rounded-lg">
          ...
          <div className="border-t-2 border-[var(--border-light)] dark:border-[var(--border-dark)] pt-6 space-y-4">
              <h3 className="text-lg font-bold text-gray-700 dark:text-gray-200">افزودن بازیکن جدید</h3>
              <div className="flex gap-2">
                  <input type="text" value={newPlayerAvatar} onChange={e => setNewPlayerAvatar(e.target.value)} placeholder="👤" className="w-16 text-center px-2 py-2 bg-white/50 dark:bg-gray-800/50 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition" />
                  <input type="text" value={newPlayerName} onChange={e => setNewPlayerName(e.target.value)} placeholder="نام بازیکن جدید" className="flex-grow w-full px-4 py-2 bg-white/50 dark:bg-gray-800/50 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition dark:placeholder-gray-400" />
          ...
          <button onClick={onClose} className="w-full mt-8 bg-gray-500/20 dark:bg-gray-600/40 text-gray-800 dark:text-gray-100 font-bold py-3 rounded-lg hover:bg-gray-500/30 dark:hover:bg-gray-600/60 transition">بستن</button>
        </div>
      </div>
